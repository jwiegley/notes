...
Lemma vapp_cons {n m} (v : Vec n) (u : Vec m) a :
  vapp (vcons a v) u = vcons a (vapp v u).
Proof. case: n => [|n] in v *; case: v => //. Qed.

(*
Lemma vapp_nil {n} (v : Vec n) : vapp v vnil = v.
Proof.
  induction n as [|n].
    case: v.
    case: (eq_rect_r _ _ _) => //.
  destruct v as [a v].
  rewrite vapp_cons {}IHn.
  destruct n, v; rewrite /vcons /=.
    admit.
  admit.
Admitted.

Lemma vapp_to_app {n m} (v : Vec n) (u : Vec m) :
  vec_to_seq (vapp v u) = vec_to_seq v ++ vec_to_seq u.
Proof.
  induction n as [|n].
    case: v => //=.
  case: m => [|m] in u IHn *.
