...
-- | A 'GHC.Generics.Generic' 'Traversal' that visits every occurrence
-- of something 'Typeable1' anywhere in a container.
--
-- >>> traverse tinplate1 listToMaybe ([1::Int],[2::Double],(),"Hello",["Hello"])
-- (Just 1,Just 2.0,(),"Hello",Just "Hello")
tinplate1 :: (Generic1 f, GTraversal1 (Rep1 f),
              Typeable1 (Rep1 f), Typeable1 (Rec1 f),
              Typeable1 (Rec1 (Rep1 f)),
              Typeable a) => Traversal' (f a) (Rep1 f a)
tinplate1 = generic1 . tinplated1 True
{-# INLINE tinplate1 #-}
...
maybeGArg1Of :: Maybe (c (t a)) -> (b -> f b) -> Maybe (c (t a))
maybeGArg1Of = const
{-# INLINE maybeGArg1Of #-}

-- | Used to traverse 'Generic1' data by 'uniplate1'.
class GTraversal1 f where
  tinplated1 :: (Typeable1 g, Typeable a) => Bool -> Traversal' (f a) (g a)

instance (Generic1 f, GTraversal1 (Rep1 f), Typeable1 f,
          Typeable1 (Rec1 f)) => GTraversal1 (Rec1 f) where
  -- k:   (b -> g b) ->
  -- f:   f a  [from: Rec1 f a] ->
  --      f1 (Rec1 f a)
  tinplated1 rec k (Rec1 f) = case gcast1 (Just f) `maybeGArg1Of` k of
    Just (Just b)  -> fromJust . gcast1 $ k b
    _ | rec       -> Rec1 <$> fmap generic1 (tinplated1 False) k f
      | otherwise -> pure $ Rec1 f
  {-# INLINE tinplated1 #-}
