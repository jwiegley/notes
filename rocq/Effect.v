...
Definition runPure `(c : Eff [] a) : a :=
  match c with
  | Pure x => x
  | Join x k f => False_rect _ (Union_empty_contra f)
  end.
...
Instance Abortive_Functor : Functor Abortive := {
  fmap := fun _ _ f x => match x with Exit e => Exit (f e) end
}.

Definition abort `{Handles Abortive r} : Eff r () :=
  Join Pure (Inj (Exit ())).
...
Instance Reader_Functor (e : Type) : Functor (Reader e) := {
  fmap := fun _ _ f x => match x with Ask k => Ask (f \o k) end
}.

Definition ask `{Handles (Reader e) r} : Eff r e :=
  Join Pure (Inj (Ask id)).
